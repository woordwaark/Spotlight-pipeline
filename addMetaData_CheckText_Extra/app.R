  ...
  (
    ...
    (
      ...
      (
        ...
        fluidPage(
          align = "center",
          radioButtons("status", NULL, c("te controleren", "bezig met controle", "goedgekeurd"), selected = "te controleren", inline=T)
        )
      ),
    ...
    tabPanel
    (
      title = HTML("<span id='metadata' class='menu'>Voer controle uit</span>"),
      value = "metadata",
    ...
    tabPanel
    (
      title = HTML("<span id='xml' class='menu'>Controle afronden</span>"),
      value = "finish",
...
server <- function(input, output, session)
{
  ...
  observe(
  {
    ...
    listFiles3 <- str_extract(list.files(path = "docs", pattern = "(*.ocr)|(*.OCR)", recursive = F, full.names = F), paste0("[:graph:]+(?=\\.(ocr|OCR))"))

    if (input$status=="te controleren")
    {
    ...
    if (input$status=="bezig met controle")
    {
  ...
  output$verderMeta <- renderUI(
  {
    ...
    return(fluidPage(align="center", br(style='content: " "; display: block; margin: 10px 0;'), actionLink('verderMeta', 'Voer controle uit')))
  })
  ...
  observe(
  {
    ...
    if (!is.null(input$listSources) & (input$status!="goedgekeurd"))
    {
      ...
      if (file.exists(paste0("docs/", input$listSources,".ocr")))
      {
        system(paste0("cp docs/", input$listSources, ".ocr docs/", input$listSources,".txt"))
        global$OCR <- paste0("docs/", input$listSources,".txt")
      }
      else
      ...
      if (file.exists(paste0("docs/", input$listSources,".OCR")))
      {
        system(paste0("copy docs/", input$listSources, ".OCR docs/", input$listSources,".txt"))
        global$OCR <- paste0("docs/", input$listSources,".txt")
      }
      else

      {
        ...
        email <- envelope() %>% from("woordwaark@rug.nl") %>% to("woordwaark@rug.nl") %>% reply("woordwaark@rug.nl") %>% subject("PDF in metadata-app-extra-controle niet geOCRd!") %>% text(cBericht)
  ...
  observeEvent(input$listSources,
  {
    ...
    ub_nummer <- str_extract(input$listSources, "(?<=(UB_))[:alnum:]+(?=_)")
    index <- which(bronnen$ub_nummer==ub_nummer)
  ...
  output$showButton <- renderUI(
  {
    ...
    return(
      div(
        ...
        p(HTML("Als u nu wilt stoppen en op een later moment<br>verder wilt gaan met de controle van het huidige document,<br>klik dan op 'Stoppen en uitloggen' in het menu hierboven.")),

        br(),

        p(HTML("<b>Klik enkel op de knop hieronder<br>als de controle van het document <u>helemaal</u> voltooid is.</b>")),
        ...
        p(HTML("Als u verder wilt gaan met het controleren van een ander document,<br>klik dan op 'Selecteer bron' in het menu hierboven."))
      )
    )
  })

  resultOCR <- eventReactive(input$xmlButton,
  {
    req(global$OCR, input$textEditor)
    ...
    fileName <- gsub("(.txt$|.TXT$)", ".ocr" , global$OCR)
  ...
  output$resultOCR <- renderUI(
  {
    req(resultOCR(), input$textEditor)
